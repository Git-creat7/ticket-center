...
public class TicketOrderConsistencyTest extends IntegrationTestcontainers {
    ...
    void testCancelConsumer_WhenOrderPending_ShouldCancelAndReleaseStock() {
        ...
        String eventCacheKey = RedisConstants.CACHE_EVENT_DETAIL_KEY + 1L;
        String oldEventCache = stringRedisTemplate.opsForValue().get(eventCacheKey);
        stringRedisTemplate.opsForValue().set(eventCacheKey, "core-owned-event");
        ...
        try {
            Assertions.assertEquals(baseStock, ticketService.queryTicketOfEvent(1L).stream()
                    .filter(ticket -> ticket.getId().equals(testTicketId)).findFirst().orElseThrow().getStock());
            // 模拟延时关单消息到达
            ...
            Assertions.assertEquals(baseStock + 1, ticketService.queryTicketOfEvent(1L).stream()
                    .filter(ticket -> ticket.getId().equals(testTicketId)).findFirst().orElseThrow().getStock());
            Assertions.assertEquals("core-owned-event", stringRedisTemplate.opsForValue().get(eventCacheKey),
                    "订单服务不修改活动缓存");
            ...
            stringRedisTemplate.delete(stockKey);
            stringRedisTemplate.delete(orderKey);
            if (oldEventCache == null) {
                stringRedisTemplate.delete(eventCacheKey);
            } else {
                stringRedisTemplate.opsForValue().set(eventCacheKey, oldEventCache);
            }
        }
    }
