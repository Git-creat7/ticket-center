...
public class TicketOrderProducer {
    ...
    private void publish(TicketOrderMessage message, int retryCount) {
    ...
    private void scheduleRetry(TicketOrderMessage message, int retryCount) {
        if (retryCount >= MAX_CONFIRM_RETRIES) {
            log.error("【RabbitMQ】订单消息确认连续失败，交由预约恢复任务继续处理，orderId={}", message.getId());
            return;
        }
        ...
        try {
            taskScheduler.schedule(() -> retryDelivery(message, nextRetry), retryAt);
        } catch (RuntimeException e) {
            log.error("【RabbitMQ】无法安排消息重投，交由预约恢复任务继续处理，orderId={}", message.getId(), e);
        }
    }

    private void retryDelivery(TicketOrderMessage message, int retryCount) {
        try {
