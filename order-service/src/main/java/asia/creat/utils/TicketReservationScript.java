...
public class TicketReservationScript {
    ...
    public Long reserve(Long ticketId, Long userId, Long reservationId) {
        return stringRedisTemplate.execute(RESERVATION_SCRIPT, keys(ticketId),
                userId.toString(), reservationId.toString());
    }
    ...
    public boolean isReserved(Long ticketId, Long userId, Long reservationId) {
        return reservationId.toString().equals(stringRedisTemplate.opsForHash()
                .get(RedisConstants.ticketReservationKey(ticketId), userId.toString()));
    }

    public Long rollback(Long ticketId, Long userId, Long reservationId) {
        return stringRedisTemplate.execute(RESERVATION_ROLLBACK_SCRIPT, keys(ticketId),
                userId.toString(), reservationId.toString());
    }
    ...
    private List<String> keys(Long ticketId) {
        return List.of(RedisConstants.ticketStockKey(ticketId), RedisConstants.ticketOrderKey(ticketId),
                RedisConstants.ticketReservationKey(ticketId));
    }
