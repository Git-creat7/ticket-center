...
| :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| **签到状态查询 (Bitmap)** | `GET /user/sign/status` | 200 | 10,000 | **3,373.8 req/s** | 3,349.0 ~ 3,373.8（稳态两轮） | 40 ~ 41 ms |
...
* **签到接口调优效果**：这个接口一度从基线 2,542.6 QPS 回落到约 1,015 QPS，三轮复测波动仅 7.2%，是稳定复现而非抖动。定位结论是接口按位逐次打 Redis——今日 `GETBIT`、连签 `BITFIELD`、当月 `BITCOUNT`、本周七天各一次，加上拦截器固定的两次，实测每请求 12 次往返。改成整月位图一次读回、位运算在内存里完成后，稳态 1,206 → 3,374 QPS（2.8 倍）、147 → 40 ms（降 73%），错误率两侧均为 0。
