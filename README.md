...
>
> **个人独立完成**：后端整体开发、核心抢票链路、并发控制、缓存优化、数据库设计、自动化测试，以及 CI/CD 流水线与镜像交付方案设计。
> **Agent 协助完成**：前端页面开发、部分部署脚本、压测验证、配置模板以及文档的编写整理
## 1. 项目简介 + 核心成果
...
- **量化结论**：100 并发提交 1,000 次请求争抢 50 张票，最终落库 50 笔，零超卖、零少卖；拆分后签到状态查询 3,008 ~ 3,635 req/s，演出详情 2,511 ~ 3,584 req/s，秒杀受理 728 req/s
...
- **签到状态 Redis 往返多** → 整月 Bitmap 一次读回，位运算放在内存 → 往返从约 12 次降到 3 次，稳态 3,008 ~ 3,635 req/s。
...
- **拆分后秒杀被拒请求也排行锁** → Redis Lua 预扣挪到受理最前面，只让放行的请求进事务；受理提交后即时推送建单任务 → 秒杀 223 → 728 req/s，受理到出票 225 ~ 278 ms。
...
微服务拆分后的压测（2026-09-12，同机 JMeter，经 Gateway，三轮取稳态）：

| 场景 | 并发 / 样本 | 吞吐量 | 平均响应 | p95 | 单体基线 |
| :--- | :---: | :---: | :---: | :---: | :---: |
| 签到状态查询（Bitmap） | 200 / 10,000 | 3,008 ~ 3,635 req/s | 49 ~ 60 ms | 83 ~ 97 ms | 2,860 ~ 3,005 |
| 演出详情高频读（Cache） | 200 / 4,000 | 2,511 ~ 3,584 req/s | 35 ~ 60 ms | 66 ~ 118 ms | 3,463 ~ 4,499 |
| 秒杀抢票并发写（Lua + 事务 + MQ） | 100 / 1,000 | 728 ~ 731 req/s | 97 ~ 101 ms | 311 ~ 340 ms | 1,012 ~ 1,101 |

拆分刚完成时详情只有 1,376 ~ 1,528、秒杀只有 223 ~ 243：详情命中缓存后还要经 Feign 查订单库，秒杀被拒的请求也在排票档行锁。把票档从详情页拆出、Lua 预扣挪到受理最前面之后回到上表；与单体的剩余差距来自 Gateway 一跳和受理路径保留的幂等与查重点查。同机同配置的噪声约 30%，小于这个幅度的差异不作结论。
...
- 应用层支持多服务器多实例部署：每台节点运行 Gateway、`ticket-center-api` 和 `order-service`，向同一 Nacos 注册各自的宿主机私网 IP 和映射端口，Gateway/OpenFeign 按服务名调用。`docker-compose.app-prod.yml` 独立连接共享中间件，入口负载均衡需另行配置。已在两台主机上联调通过：每个服务各两个实例注册到 Nacos，经任一 Gateway 的请求按轮询精确落到两台机器（`deploy/verify-multinode.sh` 四组各 20 次均为 10/10）；停掉一台节点后 Nacos 立即摘除该实例，另一台 Gateway 20/20 全部 200。
